...
def em_step(vc, tv, tc):
    ...
    np.savetxt('plsa_tv_K3', new_tv, delimiter=',', fmt='%10.10f')
    np.savetxt('plsa_tc_K3', new_tc, delimiter=',', fmt='%10.10f')
...
def main():
    ...
    tc = np.loadtxt('model\\plsa_tc_K3', delimiter=',')
    tv = np.loadtxt('model\\plsa_tv_K3', delimiter=',')
    gc.collect()
    time.sleep(2)
    print('123')
    em_step(vc, tv, tc)
